...
Helm also has some completion support for [[http://www.gnu.org/software/emacs/manual/html_mono/eshell.html][eshell]], which may be duplicate with
auto-complete. I only enable this feature for experiment, maybe later I'll use
auto-complete instead.

#+NAME: helm
#+BEGIN_SRC emacs-lisp
  (defun ome-helm-config ()
